...
window.MonacoEnvironment = {
    getWorkerUrl: function (moduleId, label) {
        return `./vendor/monaco-editor-0.44.0/min/vs/base/worker/workerMain.js`;
    }
};


const judge0ToMonacoMap = {
    "C (GCC 14.1.0)": "cpp",
    "C++ (GCC 14.1.0)": "cpp",
    "Python (3.11.2)": "python",
    "Java (JDK 17.0.6)": "java",
    "C# (.NET Core SDK 7.0.400)": "csharp",
    "Go (1.18.5)": "go",
    "PHP (8.3.11)": "php",
...
const lspServerUrls = {
    python: "ws://localhost:3008/", 
    cpp: "ws://localhost:3001/",   
    java: "ws://localhost:3002/",  
    csharp: "ws://localhost:3003/", 
    go: "ws://localhost:3004/",    
    php: "ws://localhost:3005/",   
    javascript: "ws://localhost:3006/", 
    typescript: "ws://localhost:3007/", 
};
...
function connectToLanguageServer(language, wsUrl) {
    ...
    webSocket.onerror = (error) => {
        console.error(`Connexion error in the LSP server for ${language} :`, error);
        showError("Error LSP", ` ${language}.`);
    };
}



const openaiKey = '';// Get yours
const deepseekKey = '';// Get yours
...
function explainCurrentCode() {
    ...
    return getAIResponse(prompt, "openai");
}

function fixCompilationError() {
    const currentCode = sourceEditor.getValue();
    const errorOutput = stdoutEditor.getValue();
    const prompt = "I received the following error:\n" + errorOutput + "\n\nPlease fix the following code:\n" + currentCode;
    return getAIResponse(prompt, "openai");
}
...
function fetchDeepSeekResponse(message) {
    ...
    .then(response => {
        if (!response.ok) {
            throw new Error("Error DeepSeek: " + response.statusText);
        }
        return response.json();
    })
    .then(data => {
...
function getAIResponse(message, engine) {
    switch(engine) {
        case "openai":
            return fetchOpenAIResponse(message);
        case "deepSeek":
            return fetchDeepSeekResponse(message);
        default:
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    $selectLanguage.change(function (event, data) {
        ...
        const monacoLang = judge0ToMonacoMap[selectedText];
        if (monacoLang) {
            monaco.editor.setModelLanguage(sourceEditor.getModel(), monacoLang);
        } else {
            console.warn("Error loading language:", selectedText);
        }
    });
    ...
    require(["vs/editor/editor.main", "monaco-lsp-bundle"], function (ignorable , lspBundle) {

        window.MonacoLanguageClient = lspBundle.MonacoLanguageClient;
        window.CloseAction = lspBundle.CloseAction;
        window.ErrorAction = lspBundle.ErrorAction;
        window.listen = lspBundle.listen;
        ...
        layout.registerComponent("source", function (container, state) {
            ...
            function setupLSPForCurrentLanguage() {
            ...
            setupLSPForCurrentLanguage();

            sourceEditor.onDidChangeModelLanguage((event) => {
               setupLSPForCurrentLanguage();
            });
        ...
        layout.registerComponent("assistant", function (container, state) {
            ...
            $container.html(`
              <div id="assistant-container">
                <div class="ai-header">
                  ...
                  <button id="fix-code">Fix Code</button>
                </div>
                <div id="assistant-chat-log" class="chat-log"></div>
                <div class="chat-input-container">
                    <textarea class="chat-input" placeholder="Ask your question here"></textarea>
                    <button class="chat-send">Send</button>
                </div>
              </div>
            `);

            const chatLog = $container.find('#assistant-chat-log')[0];
            new ResizeObserver(() => layout.updateSize()).observe(chatLog);
            
            const $textarea = $container.find(".chat-input");
            const $sendBtn = $container.find(".chat-send");
            const $chatLog = $container.find("#assistant-chat-log");
            ...
            $sendBtn.click(function () {
                ...
                if (message) {
                    $chatLog.append(`<div class="user-msg">You: ${message}</div>`);
                    $textarea.val('');
                    $textarea.css("height", "auto");
                    
                    const selectedEngine = $container.find("#ai-engine-select").val();
                    
                    getAIResponse(message, selectedEngine).then(response => {
                        $chatLog.append(`<div class="ai-msg">Assistant: ${response}</div>`);
                    }).catch(error => {
                        $chatLog.append(`<div class="error-msg">Error: ${error}</div>`);
                    });
                }
            });
            ...
            $container.find("#explain-code").click(function () {
                explainCurrentCode().then(response => {
                    $chatLog.append(`<div class="ai-msg">Explanation: ${response}</div>`);
                }).catch(error => {
                    $chatLog.append(`<div class="error-msg">Error: ${error}</div>`);
                });
            });
            
            $container.find("#fix-code").click(function () {
                fixCompilationError().then(response => {
                    $chatLog.append(`<div class="ai-msg">Fix suggestion: ${response}</div>`);
                }).catch(error => {
                    $chatLog.append(`<div class="error-msg">Error: ${error}</div>`);
                });
            });
